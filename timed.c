...
struct ValidationTimes {
    ...
    struct {
        ...
        double softmax;   // --- New: cumulative softmax time ---
    } attention;
    double residual1;
    double norm2;
    struct {
...
/* --- Fully Connected (Matrix Multiplication) ---
     Here we measure the inner matrix multiplication time per sample. --- */
static void fully_connected(const struct FullyConnectedLayer* fc) {
    double t_total = get_current_time();
    double t_matmul_total = 0.0;
    for (size_t sample = 0; sample < fc->sample_count; sample++) {
        ...
        double t_sample_start = get_current_time();
        ...
        double t_sample_end = get_current_time();
        t_matmul_total += (t_sample_end - t_sample_start);
    }
    double t_end = get_current_time();
    fprintf(stderr, "Fully connected: total: %.6f s, matrix multiplication: %.6f s\n",
            t_end - t_total, t_matmul_total);
}
...
static void process_transformer(struct ModelParameters* modelParams,
    ...
    vt.attention.softmax = 0.0;  // Initialize cumulative softmax timer
    ...
    for (int layer = 0; layer < NUM_LAYERS; layer++) {
        ...
        for (size_t head = 0; head < NUM_HEADS; head++) {
            for (size_t q = 0; q < inputSize; q++) {
                ...
                /* --- Softmax Calculation Timing --- */
                double t_softmax_start = get_current_time();
                float sumExp = 0.0f;
                for (size_t k = 0; k <= q; k++) {
                ...
                double t_softmax_end = get_current_time();
                vt.attention.softmax += (t_softmax_end - t_softmax_start);
                /* --- End Softmax Timing --- */
...
int main(void) {
    ...
    #undef ALIGN
    t1 = get_current_time();
    fprintf(stderr, "Memory allocation size calculation: %.6f s\n", t1 - t0);
    ...
    assert(memoryBlock);
    t1 = get_current_time();
    fprintf(stderr, "Memory allocated in: %.6f s\n", t1 - t0);
    ...
    t1 = get_current_time();
    fprintf(stderr, "Token decoder loaded in: %.6f s\n", t1 - t0);
    ...
    t1 = get_current_time();
    fprintf(stderr, "Tokenized data loaded in: %.6f s\n", t1 - t0);
    ...
    t1 = get_current_time();
    fprintf(stderr, "Safetensor parameters loaded in: %.6f s\n", t1 - t0);
    ...
    if (isTraining) {
        ...
        t1 = get_current_time();
        fprintf(stderr, "Transformer process (training) took: %.6f s\n", t1 - t0);
    } else {
        ...
        for (int i = 0; i < 128; i++) {
            ...
            t1 = get_current_time();
            fprintf(stderr, "Transformer inference (iteration %d) took: %.6f s\n", i, t1 - t0);
